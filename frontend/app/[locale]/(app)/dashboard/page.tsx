...
// the current date rather than an arbitrary calendar boundary.
function bucketByWeek(trend: DashboardOverviewResponse["ticket_trend"]): WeekBucket[] {
...
function kpiStatus(breachedTickets: number, totalTickets: number): { label: string; tone: string } {
  if (totalTickets === 0) return { label: "No Data", tone: "text-ink-500" };
  const breachRate = breachedTickets / totalTickets;
  if (breachRate <= 0.05) return { label: "On Track", tone: "text-ink-950" };
  if (breachRate <= 0.15) return { label: "At Risk", tone: "text-amber-600" };
  return { label: "Off Track", tone: "text-red-600" };
}
...
export default function DashboardPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(apiErrorMessage(err, "Couldn't load dashboard metrics"));
    } finally {
      setLoading(false);
    }
  }
  ...
  const weeks = useMemo(() => (overview ? bucketByWeek(overview.ticket_trend) : []), [overview]);
  ...
  return (
    <div>
      <PageHeader title="Dashboard" description="Live snapshot of ticket volume, SLA health, and recent trend." />

      {loading && <p className="px-1 py-8 text-center text-sm text-ink-500">Loading…</p>}
      ...
      {overview && !loading && !error && (
        <>
          ...
          <div className="mt-6 rounded-card border border-ink-100 bg-white p-6">
            <h2 className="text-sm font-semibold text-ink-950">Ticket Volume — Last {WEEKS} Weeks</h2>
                ...
                return (
                  <div key={week.label} className="flex flex-1 flex-col items-center gap-2">
                    <div
                      ...
                      style={{ height }}
                      title={`${week.label}: ${week.total} tickets`}
                    />
