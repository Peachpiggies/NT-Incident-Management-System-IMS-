...
from app.db.models import Base, Permission, RefreshToken, Role, RolePermission, User, UserRole
...
def test_auth_rotation_and_reuse_revokes_all_sessions(tmp_path) -> None:
    async def scenario() -> None:
        ...
        async with sessions() as session:
            ...
            created = await register(UserRegisterRequest(email="customer@example.com", full_name="Test Customer", password="secure-password-123"), session)
            first_refresh = created.refresh_token
            rotated = await refresh(RefreshRequest(refresh_token=first_refresh), session)
            ...
            active = await session.scalars(select(RefreshToken).where(RefreshToken.revoked_at.is_(None)))
            assert list(active) == []
...
def test_database_permission_check_allows_only_granted_user(tmp_path) -> None:
    async def scenario() -> None:
        ...
        async with sessions() as session:
            ...
            permission = Permission(module="ticket", action="assign", code="ticket.assign")
            allowed = User(username="allowed", email="allowed@example.com", first_name="Allowed", last_name="User", password_hash="hash")
            denied = User(username="denied", email="denied@example.com", first_name="Denied", last_name="User", password_hash="hash")
            session.add_all([role, permission, allowed, denied])
            await session.flush()
            session.add_all([RolePermission(role_id=role.id, permission_id=permission.id), UserRole(user_id=allowed.id, role_id=role.id)])
            await session.commit()
