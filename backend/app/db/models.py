...
# the Problem Management migration via `op.create_foreign_key` against

# the already-existing `root_causes`/`rca_reports` tables rather than

# recreating either of them -- see that migration's docstring.
...
# Problem Management
# ==========================================================
# A Problem is the underlying cause behind one or more Incidents (Tickets).
# `status` is a fixed five-value workflow (OPEN -> UNDER_INVESTIGATION ->
# KNOWN_ERROR -> RESOLVED -> CLOSED, with UNDER_INVESTIGATION able to close
# straight to RESOLVED if no workaround was ever needed) -- unlike
# TicketStatus/KBArticleStatus this isn't configurable master data,
# matching the fixed `ProblemStatus` enum already declared in
# app.schemas.problem. See app/services/problem.py for the transition
# table, which mirrors app/services/rca.py's approach for RCAReport.


class Problem(BaseModel):
    __tablename__ = "problems"
    __table_args__ = (
        UniqueConstraint("problem_no", name="uq_problems_problem_no"),
        CheckConstraint(
        ...
        Index("ix_problems_category_priority", "category_id", "priority_id"),
    )

    problem_no: Mapped[str] = mapped_column(String(32), nullable=False, index=True)
    title: Mapped[str] = mapped_column(String(255), nullable=False)
    description: Mapped[str] = mapped_column(Text, nullable=False)
    category_id: Mapped[UUID] = mapped_column(
        Uuid, ForeignKey("ticket_categories.id"), nullable=False, index=True
    ...
    incident_links: Mapped[list[ProblemIncidentLink]] = relationship(
        back_populates="problem"
    )
    workarounds: Mapped[list[Workaround]] = relationship(back_populates="problem")
    known_errors: Mapped[list[KnownError]] = relationship(back_populates="problem")
    permanent_fixes: Mapped[list[PermanentFix]] = relationship(
        back_populates="problem"
    )


class ProblemNumberSequence(Base):
    """One locked counter per UTC day for human-friendly problem numbers.
    Mirrors TicketNumberSequence."""

    __tablename__ = "problem_number_sequences"
    business_date: Mapped[date] = mapped_column(Date, primary_key=True)
    last_value: Mapped[int] = mapped_column(Integer, nullable=False, default=0)


class ProblemIncidentLink(BaseModel):
    """Links a Problem to one of the Incidents (Tickets) it caused."""

    __tablename__ = "problem_incident_links"
    __table_args__ = (
        UniqueConstraint(
            "problem_id", "ticket_id", name="uq_problem_incident_links_pair"
        ),
        Index("ix_problem_incident_links_ticket_id", "ticket_id"),
    )

    problem_id: Mapped[UUID] = mapped_column(
        Uuid, ForeignKey("problems.id"), nullable=False, index=True
    )
    ticket_id: Mapped[UUID] = mapped_column(
    ...
    problem: Mapped[Problem] = relationship(back_populates="incident_links")
    ticket: Mapped[Ticket] = relationship()
    linked_by: Mapped[User] = relationship(foreign_keys=[linked_by_id])
...
class Workaround(BaseModel):
    ...
    __table_args__ = (
        ...
        Index("ix_workarounds_problem_id", "problem_id"),
    )
    ...
    problem: Mapped[Problem] = relationship(back_populates="workarounds")
    # `BaseModel.created_by` is the generic audit FK; `creator` is the same
    # FK exposed as a relationship so app.schemas.problem.WorkaroundResponse
    # can nest the authoring user under `created_by` without colliding with
    # the raw audit column. See that schema's `_map_creator` validator.
    creator: Mapped[User | None] = relationship(foreign_keys="Workaround.created_by")


class KnownError(BaseModel):
    """A problem enters the Known Error Database (KEDB) once its root
    cause and (usually) a workaround are documented but a permanent fix
    isn't in place yet -- see Problem.status == KNOWN_ERROR."""

    __tablename__ = "known_errors"
    __table_args__ = (Index("ix_known_errors_problem_id", "problem_id"),)

    problem_id: Mapped[UUID] = mapped_column(
        Uuid, ForeignKey("problems.id"), nullable=False
    )
    symptoms: Mapped[str] = mapped_column(String(4000), nullable=False)
    root_cause_summary: Mapped[str | None] = mapped_column(String(4000))
    workaround_id: Mapped[UUID | None] = mapped_column(
        Uuid, ForeignKey("workarounds.id")
    )
    is_published_to_kb: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False
    )
    kb_article_id: Mapped[UUID | None] = mapped_column(
        Uuid, ForeignKey("kb_articles.id")
    )

    problem: Mapped[Problem] = relationship(back_populates="known_errors")
...
class PermanentFix(BaseModel):
    ...
    Change Management only has Pydantic schemas (app.schemas.change_management)
    and no backing ORM table/migration. Add the FK once that migration
    lands, the same way root_causes.problem_id / rca_reports.problem_id
    gained their FK to `problems` here."""

    __tablename__ = "permanent_fixes"
    __table_args__ = (Index("ix_permanent_fixes_problem_id", "problem_id"),)
    ...
    problem: Mapped[Problem] = relationship(back_populates="permanent_fixes")
